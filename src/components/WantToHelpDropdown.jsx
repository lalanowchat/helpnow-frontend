  ...
  return (
    ...
    >
      ...
      <CardContent className="text-xs space-y-1 pt-0">
        {phone && tel && (
          <a href={tel} className="block text-emerald-600 hover:underline" onClick={(e) => e.stopPropagation()}>
            {phone}
          </a>
        )}
        {website && href && (
          <a href={href} target="_blank" rel="noopener noreferrer" className="block text-emerald-600 hover:underline truncate" onClick={(e) => e.stopPropagation()}>
            {website}
          </a>
        )}
 ...
 * WantToHelpDropdown
 *
 * Identical in structure to NeedHelpDropdown but hits the want-to-help endpoint.
 *
 * Props:
 ...
 */
export default function WantToHelpDropdown({ onResults, onSelectOrg, selectedOrgId }) {
  ...
  const fetchResources = async (size, { mode = 'replace' } = {}) => {
    ...
    try {
      ...
      // TODO: replace with want-to-help endpoint once available
      ...
      console.error('WantToHelpDropdown fetch error:', err);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500 transition-colors duration-150 shrink-0">
          <input
            type="text"
            inputMode="numeric"
            maxLength={5}
          ...
          <button
            ...
            className="flex items-center justify-center w-8 h-8 bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors shrink-0"
          >
            {loading
              ? <Loader2 className="w-4 h-4 animate-spin" />
